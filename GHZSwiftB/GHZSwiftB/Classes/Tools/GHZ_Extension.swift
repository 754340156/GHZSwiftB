...
import UIKit
@objc public protocol GHZ_ExtensionProtocol {
    static func customClassMapping() -> [String: String]?
}
public class GHZ_Extension: NSObject {
    private static let instance = GHZ_Extension()
    /// 全局统一访问入口
    public class var sharedManager: GHZ_Extension {
        return instance
    }
        ...
        autoreleasepool {
            ...
            for (k, v) in infoDict {
                ...
                if let value: AnyObject = dict[k] {
                    ...
                    if v.isEmpty {
                        if !(value === NSNull()) {
                            if k == "number" && GHZScreenWidth < 375 {
    ...
    public func objectDictionary(obj: AnyObject) -> [String: AnyObject]? {
        ...
        for (k, v) in infoDict {
            ...
            if v.isEmpty || value === NSNull() {
                ...
                if type == "NSArray" {
                    subValue = objectArray(array: value! as! [AnyObject])
                } else {
                    subValue = objectDictionary(obj: value!)
                }
                if subValue == nil {
                    subValue = NSNull()
                }
